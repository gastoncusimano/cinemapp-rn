...
export default function UpcomingMovies(props,{navigation}) {
  const [state, setState] = useState({movies: [], page: 1, refreshing: false, isLoading: true})
    const initializedData = async () => {
        await Fetch.GET(`movie/upcoming?${API_KEY}&language=${API_LANG}&page=${state.page}`, (response) => response)
              .then((response) => {
                if (response) {
    ...
    return (
      <View style={styles.container}>
        { state.movies.length > 0 &&
          <SliderHorizontal sliderTitle="Upcoming Movies" movies={state.movies} loadMore={loadMore} navigation={props.navigation} isRefreshing={state.refreshing} moreDelay={props.moreDelay}/>
        }
      </View>
    );
}
